    ...
    <div class='col-md-9' id="content">
        ...
        <div class='panel'>
            <div class='header topic-header'>
                ...
                <div class="changes">
                    <span>${article.topic.friendlyTime}</span><span>&nbsp;&nbsp;作者：<a href="${contextPath}/pub/user/${article.topic.authorId}">${article.topic.authorName}</a></span><span>&nbsp;&nbsp; ${article.topic.visitCount}次流覽</span>
        ...
        <c:if test="${not empty article.replyList}">
            <div class="panel" id="comment">
                <div class="header">
                    <span class="col_fade">${article.topic.replyCount} 回復</span>
                </div>
                    ...
                    <div class="cell reply_area reply_item editormd-preview-container" reply_id="${reply.id}" reply_to_id="" id="${reply.id}">
                        <div class="author_content">
                            ...
                            <div class="user_info">
                                ...
                                <a class="reply_time" href="#${reply.id}">${status.index+1}樓•${reply.friendlyTime}</a>
                            </div>
                            <div class="user_action">
                              <span>
                                <i class="fa up_btn fa-thumbs-o-up invisible" title="喜歡">
                                ...
                                <c:if test="${not empty loginUser}">
                                    <c:if test="${loginUser.id==reply.authorInfo.authorId}">
                                        <a href="javascript:;" class="edit-reply" reply-id="${reply.id}" title="編輯"><i class="glyphicon glyphicon-edit"></i></a>
                                        <a href="${contextPath}/article/${article.topic.id}/reply/${reply.id}/del/" title="刪除"><i class="glyphicon glyphicon-remove-circle"></i></a>
                                    </c:if>
                                </c:if>
                            </div>
                        </div>
            ...
            <div id="reply">
                <form action="${contextPath}/reply/add/${article.topic.id}" method="post" class="form-vertical" id="reply-form" role="form">
                    <fieldset>
                        ...
                        <input type="submit" class="btn btn-primary" value="回復" id="submit">
                    </fieldset>
                </form>
            </div>
                ...
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        <div class="modal-header">
                            ...
                            <h4 class="modal-title" id="myModalLabel">編輯評論</h4>
                        </div>
        ...
        <c:if test="${empty loginUser}">
            <div class="content" style="padding: 2em;">
                需要 <a href="${contextPath}/login" class="btn btn-primary">登錄</a> 後方可回復, 如果你還沒有帳號你可以 <a href="${contextPath}/register" class="btn btn-danger">註冊</a> 一個帳號。
            </div>
        </c:if>
    </div>
